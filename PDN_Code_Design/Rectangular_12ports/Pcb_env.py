...
source_file = user_name + r"\PI_PG_WI2324\Square_12ports\Batch_12Decaps_Values.peb"
destination_file = user_name + r"\PI_PG_WI2324\Square_12ports\tmp_Batch_12Decaps_Values.peb"
...
def cal_reward(state,agent):
    ...
    if agent == 0:
        with open(user_name + r"\PI_PG_WI2324\Square_12ports\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC1.csv", newline='') as csvfile:
    ...
    if agent == 1:
        with open(user_name + r"\PI_PG_WI2324\Square_12ports\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC2.csv", newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=';')
            next(reader, None)
            current_impedance_list = list(reader)
