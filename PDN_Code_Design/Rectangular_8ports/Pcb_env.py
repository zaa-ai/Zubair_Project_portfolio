...
user_name = r"C:\Users\emago\PI_MARL"
source_file = user_name + r"\PI_PG_WI2324\Rectangular_8ports\Batch_8Decaps_Values.peb"
destination_file = user_name + r"\PI_PG_WI2324\Rectangular_8ports\tmp_Batch_8Decaps_Values.peb"
state_res = user_name + r"\PI_PG_WI2324\Rectangular_8ports"
...
pos_list = np.array([1,2,3,4,5,6,7,8])
...
####################################################
#In the case of 8 ports boards we can use 2 different values for the decaps
#uncomment the type list wit 2 elements to make the change 
############################################################################
type_list = np.array([1])
# type_list = np.array([1,2])
...
def cal_reward(state,agent):
    ...
    if agent == 0:
        with open(user_name + r"\PI_PG_WI2324\Rectangular_8ports\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC1.csv", newline='') as csvfile:
    ...
    if agent == 1:
        with open(user_name + r"\PI_PG_WI2324\Rectangular_8ports\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC2.csv", newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=';')
            next(reader, None)
            current_impedance_list = list(reader)
...
class PCBEnvironment(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        # if done:
        #     info_terminated = {"message": "Environment reached terminal state"}
        # if (self.count == state_einheit or self.count == len(pos_list)) and done == False:
        #     info_truncated = {"message": "Environment truncated before reaching terminal state"}
        #     Reward_value1 = -2
        #     Reward_value2 = -2
 ...
 ####################################################################       
        if terminateds["__all__"]:
            ...
            csv_file_path = user_name + r"\PI_PG_WI2324\Rectangular_8ports\Qmix_termination_state_152_R.csv"
