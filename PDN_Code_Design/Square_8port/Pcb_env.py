...
source_file = user_name + r"\PI_PG_WI2324\Square_8port\Batch_8Decaps_Values.peb"
destination_file = user_name + r"\PI_PG_WI2324\Square_8port\tmp_Batch_8Decaps_Values.peb"
...
def cal_reward(state,agent):
    ...
    if agent == 0:
        with open(user_name + r"\PI_PG_WI2324\Square_8port\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC1.csv", newline='') as csvfile:
    ...
    if agent == 1:
        with open(user_name + r"\PI_PG_WI2324\Square_8port\Design1.emc\PI-1/Power_GND/1-PIPinZ_IC2.csv", newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=';')
            next(reader, None)
            current_impedance_list = list(reader)
...
class PCBEnvironment(MultiAgentEnv):
 ...
 ####################################################################       
        if terminateds["__all__"]:
            ...
            csv_file_path = user_name + r"\PI_PG_WI2324\Square_8port\Qmix_termination_state_150.csv"
